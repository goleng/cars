...
# and `pip install cars` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "cars", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n cars -f environment.docker.yml
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "cars", "YOUR_CONSOLE_SCRIPT"]
